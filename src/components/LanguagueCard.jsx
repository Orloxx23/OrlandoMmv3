...
export default function LanguagueCard() {
  ...
  useEffect(() => {
    ...
    if (item) {
      setLanguage(item);
      moment.locale(item); // Make sure to set the locale for moment.js on initial load.
      i18n.changeLanguage(item);
    } else {
      localStorage.setItem("language", "en");
      setLanguage("en");
      moment.locale("en"); // Set the default locale for moment.js
  ...
  const toggleLanguage = () => {
    ...
    moment.locale(newLanguage); // Set moment.js locale when changing the language.
    i18n.changeLanguage(newLanguage);
    localStorage.setItem("language", newLanguage); // Update the language in local storage.
  };
